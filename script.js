document.addEventListener('DOMContentLoaded', () => {
    ...
    const getFrequentlyOrdered = () => {
        ...
        const usageCount = {};
        history.slice(0, 50).forEach(order => {
        ...
        elements.historyListDiv.innerHTML = filteredHistory.length === 0 ? '<p class="text-center">No records for selected dates.</p>' : filteredHistory.map(item => {
            ...
            <div class="history-list-item">
                ...
                <div>
                    ...
                    ${totalCrates > 0 ? `<small style="display: block; opacity: 0.7;">Crates: ${totalCrates}</small>` : ''}
                </div>
        ...
        switch(format) {
            ...
            case 'pdf':
                try {
                    ...
                    const doc = new jsPDF();
                    const tableHead = [['Date & Time', 'Product Details', 'Qty', 'Rate', 'Total']];
                    const tableBody = [];

                    filteredHistory.forEach(order => {
                        const dateStr = new Date(order.date).toLocaleString('en-GB');
                         ...
                         tableBody.push([
                            { content: `Order Total: ₹${order.total.toFixed(2)}`, colSpan: 5, styles: { fillColor: '#e2e8f0', textColor: '#000', fontStyle: 'bold', halign: 'right' } }
                         ]);
                    });

                    doc.text("Amul Daily Order History", 14, 15);
                    doc.autoTable({
                        head: tableHead,
                        body: tableBody,
                        startY: 20,
            ...
            case 'csv':
                ...
                filteredHistory.forEach(order => {
                    ...
                    order.items.forEach(item => {
                        ...
                        csvContent += `${date},${time},${name},${size},${price},${qty},${itemTotal},${order.total.toFixed(2)}\n`;
                    });
                });
